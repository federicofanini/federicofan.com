 ...
 */
export async function fetchGitHubContributions(username: string): Promise<{
  success: boolean;
  data: GitHubContributionData | null;
  error: string | null;
}> {
  try {
    if (!username) {
      return { success: false, data: null, error: "INVALID_INPUT" };
    }
    ...
    if (!githubToken) {
      ...
      return { success: false, data: null, error: "GITHUB_TOKEN_MISSING" };
    }
    ...
    if (!response.ok) {
      ...
      return { success: false, data: null, error: "GITHUB_API_ERROR" };
    }
    ...
    if (data.errors || !data.data?.user) {
      ...
      return { success: false, data: null, error: "GITHUB_USER_NOT_FOUND" };
    }
    ...
    return { success: true, data: githubData, error: null };
  } catch (error) {
    console.error("❌ GitHub fetch error:", error);
    return { success: false, data: null, error: "GITHUB_FETCH_ERROR" };
  }
}
